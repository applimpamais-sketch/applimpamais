...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Detectar dispositivos móveis (touch) ou telas pequenas
    const isCoarsePointer =
      window.matchMedia('(pointer: coarse)').matches || window.innerWidth < 768;

    setIsMobile(isCoarsePointer);

    // Em mobile, não ativa o efeito glow pesado
  ...
  const getInlineStyles = () => {
    const baseStyles: React.CSSProperties = {
      ...
      ['--size' as any]: '200',
      ['--outer' as any]: '1',
      ['--saturation' as any]: '100', // Saturação máxima para azul vibrante
      ['--lightness' as any]: '60', // Lightness ajustado para azul mais intenso
